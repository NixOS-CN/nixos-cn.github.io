...
多行字符串往往会带有不同程度的缩进，会被一步处理。也就是说对于以下字符串：
...
属性集是用大括号（{ }）括起来的名称-值对（称为属性）的集合。

属性名可以是标识符或字符串。标识符必须以字母（a-z、A-Z）或下划线（\_）开头，也可以包含字母（a-z、A-Z）、数字（0-9）、下划线（\_）、撇号（'）或破折号（-）。

名称和值之间用等号 (=) 分隔。每个值都是一个任意表达式，以分号 (;) 结尾。
