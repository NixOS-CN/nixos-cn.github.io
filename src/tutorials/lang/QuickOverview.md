...
- 注意语法细节：`a` 后面有一个分号 `;`，而 `with` 表达式的作用域为分号后的第一个表达式。
...
可以看到，`with` 表达式具有一种“就近性”，当 `x` 的值可以不经嵌套地直接访问时，它会直接返回这个值，而不会使用 `a` 来嵌套地访问。
...
下面将要介绍的 `inherit` 语法，则简化了这种继承所需的“名称-值”对。
上面的例子可以这样写（求值结果不变）：
```nix
let
  a = 1;
...
<!-- prettier-ignore -->
::: tip
请注意 `inherit` 的语法结构，例如上面的 `inherit x;`，本质上是“名称-值”对，不属于表达式。
...
```
对其严格求值的结果不变。
可见，利用 `inherit`，我们变相地提升了 `{ x = 1; y = 2; }` 中属性的“嵌套级别”，从而省去了属性访问。
...
<!-- prettier-ignore -->
::: warning 名称的值的类型
字符串插值语法支持的值必须为字符串类型，或是可以转换为字符串的类型。例如：
