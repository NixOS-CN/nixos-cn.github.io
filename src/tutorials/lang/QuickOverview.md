...
<!-- prettier-ignore -->
::: tip 基础要求
以下教程需要你具有一定基础。具体来说，如果你已经知道在编程领域什么是变量（variable）、赋值（assign）、字符串（string）、函数（function）及参数（argument），那么你的知识水平就差不多足够了。
...
另外，本教程中的示例代码一般不是为了供直接运行而写的，对于每一段代码，若想实践其效果，请先理解对应的知识，再基于这段代码自己编写测试代码以运行。
