...
## 名称与集合
与大多数编程语言类似，变量与值是 Nix 语言中最基础的概念。本节将会介绍 Nix 中如何将名称**分配给值**，以及最常用的数据类型——**属性集**，继而引出**递归属性集**与**列表**的概念。
...
<!-- prettier-ignore -->
::: info 名称与其它语言中变量的区别
在很多语言中，值是被分配给变量的（即赋值），且往往可以重复赋值来改变变量的状态；而 Nix 语言中，名称是被分配给值的，一旦分配（定义）就无法改变。

不过，以当前的 Nix 版本来看，将 Nix 中的名称称为变量也没问题。
...
- 集合以 `{` `}` 为边界，其内部为多个“名称-值”对，且它们末尾必须添加 `;` 。
